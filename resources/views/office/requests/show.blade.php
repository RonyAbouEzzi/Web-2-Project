@extends('layouts.app')
@section('title', 'Request ' . $request->reference_number)
    ...
    <div class="card-body">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.75rem;flex-wrap:wrap">
            <div style="min-width:0">
                ...
                <h5 style="font-size:.95rem;font-weight:800;color:#111827;margin:0 0 .25rem">{{ $request->service->name }}</h5>
                <div style="display:flex;flex-wrap:wrap;gap:.5rem;align-items:center">
                    <span style="font-size:.78rem;color:#6b7280"><i class="bi bi-person me-1"></i>{{ $request->citizen->name }}</span>
            ...
            <span class="sbadge s-{{ $request->status }}" style="font-size:.78rem;padding:.3rem .8rem;flex-shrink:0">
        ...
        <div style="background:#f8fafc;border-radius:8px;padding:.65rem .85rem;margin-top:.75rem;font-size:.8rem;color:#374151;border-left:3px solid var(--primary)">
            <strong>Citizen Note:</strong> {{ $request->notes }}
        </div>
        @endif
    </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr;gap:1rem" class="req-grid">
    ...
    <div style="display:flex;flex-direction:column;gap:1rem">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('office.requests.status', $request) }}" method="POST">
                    @csrf @method('PATCH')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="office_notes" class="form-control" rows="2" placeholder="Internal notes, not shown to citizen...">{{ $request->office_notes }}</textarea>
                    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div style="display:flex;justify-content:space-between;margin-bottom:.5rem;font-size:.82rem">
                    ...
                    <span style="font-weight:700">${{ number_format($request->service->price, 2) }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;margin-bottom:.5rem;font-size:.82rem">
                    <span style="color:#6b7280">Method</span>
                    <span>{{ ucfirst($request->payment_method ?? 'N/A') }}</span>
                </div>
                <div style="display:flex;justify-content:space-between;font-size:.82rem">
                    <span style="color:#6b7280">Status</span>
                    <span class="sbadge s-{{ $request->payment_status }}">{{ ucfirst($request->payment_status) }}</span>
                </div>
                ...
                <div style="margin-top:.6rem;padding:.5rem;background:#f8fafc;border-radius:6px">
                    ...
                    <code style="font-size:.72rem">{{ $request->transaction_id }}</code>
                </div>
                @endif
            </div>
        </div>
            ...
            <div class="card-body">
                <div style="display:flex;align-items:center;gap:.65rem;margin-bottom:.75rem">
                    <div style="width:38px;height:38px;border-radius:50%;background:var(--primary-light);color:var(--primary);display:flex;align-items:center;justify-content:center;font-size:.9rem;font-weight:700;flex-shrink:0">
                        {{ strtoupper(substr($request->citizen->name,0,1)) }}
                    </div>
                    <div>
                        <div style="font-size:.85rem;font-weight:700">{{ $request->citizen->name }}</div>
                        <div style="font-size:.73rem;color:#9ca3af">{{ $request->citizen->email }}</div>
                    </div>
                </div>
                @if($request->citizen->phone)
                <div style="font-size:.79rem;color:#6b7280"><i class="bi bi-telephone me-1"></i>{{ $request->citizen->phone }}</div>
                @endif
            </div>
        </div>
    </div>
...
async function send() {
    ...
    try {
        const r = await fetch('{{ route('office.messages.send', $request) }}', {
            method: 'POST',
        ...
        if (r.ok) {
            ...
            d.innerHTML = `<div class="msg-av av-me">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</div><div class="msg-bubble"><p>${body}</p><div class="msg-time">Just now</div></div>`;
